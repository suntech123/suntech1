...
def extract_table_structure_final(image_path_or_array, save_path=None):
    ...
    # PRESERVATION 1: Relaxed Threshold (C=4) for faint gray headers
    thresh = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, 
                                   cv2.THRESH_BINARY_INV, 15, 4)

    # PRESERVATION 2: Sobel Edge Detection for colored/gray blocks
    ...
    # Preservation: Small vertical smear (1%) to handle broken lines but separate boxes
    ...
    for cnt in candidates:
        ...
        # --- NEW SAFE LOGIC: COUNT SPANNING LINES ---
        # A Structural Line spans almost the full width of the cluster (> 85%)
        # Text usually has margins or is short.
        spanning_h_count = 0
        for h_c in local_h_cnts:
            _, _, lw, _ = cv2.boundingRect(h_c)
            if lw > (w * 0.85): # 85% width threshold
                spanning_h_count += 1
        
        # RULE A: Internal Structure
        # 1. Has >= 3 Spanning Horizontal lines (Table)
        # OR
        # 2. Has >= 3 Vertical lines (Columnar Table)
        # (Text box has 2 spanning H-lines and 2 V-lines -> Fails)
        has_internal_structure = (spanning_h_count >= 3) or (n_v >= 3)

        # RULE B: List Table (No Verticals)
        # Must have spanning lines (header/row seps) and be wide
        relative_width = w / img.shape[1]
        is_list_table = (n_v < 2) and (spanning_h_count >= 3) and (relative_width > 0.5)
        
        # RULE C: Grid Check
        roi_intersect = cv2.bitwise_and(roi_h, roi_v)
        num_joints = len(cv2.findContours(roi_intersect, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)[0])
        
        # PRESERVATION 3: Connectivity Check (0.4 Threshold)
        # This keeps the sectioned table working (broken vertical lines)
        is_connected = True
        if num_joints >= 4 and n_v >= 2:
            ...
            if max_v_len < (h * 0.4): is_connected = False # 40% height requirement
